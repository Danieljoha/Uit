...
```{python}  
a, b, Q_O, Q_C, c, f = sp.symbols('a b Q_O Q_C, c, f')

def P_demand(Q):
    return a-b*Q


def profit_1(Q_O,Q_C):
    return (P_demand(Q_O+Q_C)-c)*Q_O-f

def profit_2(Q_O,Q_C):
    return (P_demand(Q_O+Q_C)-c)*Q_C-f

derivert_1 = sp.diff(profit_1(Q_O,Q_C), Q_O)
derivert_2 = sp.diff(profit_2(Q_O,Q_C), Q_C)

eq1 = sp.Eq(derivert_1, 0).subs({a:990, b:1/60, c:50, f:3000000})
eq2 = sp.Eq(derivert_2, 0).subs({a:990, b:1/60, c:50, f:3000000})

kvantum = sp.solve((eq1, eq2), (Q_O, Q_C))

markedspris = P_demand(kvantum[Q_O]+kvantum[Q_C])


print(f'''Optimalt kvantum for Q_O er {round(kvantum[Q_O])} og for Q_C er {round(kvantum[Q_C])}. \nSå det totale kvantumet blir {round(kvantum[Q_O]+kvantum[Q_C])} og markedsprisen blir {round(markedspris.subs({a:990, b:1/60}), 2)}''')

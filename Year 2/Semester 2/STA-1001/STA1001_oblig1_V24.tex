...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\#integrate(function(x) (2/5)*x*exp({-}1/5*x\^{}2), 0, Inf)}
\FunctionTok{integrate}\NormalTok{(dford2,}\AttributeTok{lower=}\DecValTok{0}\NormalTok{,}\AttributeTok{upper=}\ConstantTok{Inf}\NormalTok{)}\SpecialCharTok{$}\NormalTok{value}
\end{Highlighting}
\end{Shaded}
...
\paragraph{Finn den kumulative fordelingsfunksjonen, ( F(x) ), og
skissér også
denne.}\label{finn-den-kumulative-fordelingsfunksjonen-fx-og-skissuxe9r-ogsuxe5-denne.}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\#curve(1 {-} exp({-}1/5*x\^{}2), from = 0, to = 5)}

\CommentTok{\# pford1 = function(x) \# "p" for "(cumulative) probablility".}
\CommentTok{\# \{kum = vector(length=length(x))}
\CommentTok{\# for (i in 1:length(x))}
\CommentTok{\# \{if (x[i]\textgreater{}0) kum[i] = 1 {-} (x[i]+1)*exp({-}x[i])}
\CommentTok{\# else kum[i] = 0\}}
\CommentTok{\# return(kum)\}}


\NormalTok{pford2 }\OtherTok{=} \ControlFlowTok{function}\NormalTok{(x) }\CommentTok{\# "p" for "(cumulative) probablility".}
\NormalTok{  \{kum }\OtherTok{=} \FunctionTok{vector}\NormalTok{(}\AttributeTok{length=}\FunctionTok{length}\NormalTok{(x))}
  \ControlFlowTok{for}\NormalTok{ (i }\ControlFlowTok{in} \DecValTok{1}\SpecialCharTok{:}\FunctionTok{length}\NormalTok{(x))}
\NormalTok{  \{}\ControlFlowTok{if}\NormalTok{ (x[i]}\SpecialCharTok{\textgreater{}}\DecValTok{0}\NormalTok{) kum[i] }\OtherTok{=} \FunctionTok{integrate}\NormalTok{(dford2, }\AttributeTok{lower=}\DecValTok{0}\NormalTok{, }\AttributeTok{upper =}\NormalTok{ x[i])}\SpecialCharTok{$}\NormalTok{value }
  \ControlFlowTok{else}\NormalTok{ kum[i] }\OtherTok{=} \DecValTok{0}\NormalTok{\}}
  \FunctionTok{return}\NormalTok{(kum)\}}

\CommentTok{\# curve(pford1,from=0,to=10,xlab="x",ylab="F(x)",main="Kumulativ fordelingsfunksjon",col="blue")}
\FunctionTok{curve}\NormalTok{(pford2,}\AttributeTok{from=}\DecValTok{0}\NormalTok{,}\AttributeTok{to=}\DecValTok{5}\NormalTok{,}\AttributeTok{xlab=}\StringTok{"x"}\NormalTok{,}\AttributeTok{ylab=}\StringTok{"F(x)"}\NormalTok{,}\AttributeTok{main=}\StringTok{"Kumulativ fordelingsfunksjon"}\NormalTok{,}\AttributeTok{col=}\StringTok{"blue"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

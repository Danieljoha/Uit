...
title: "SOK-1004, høst 2022, Arbeidskrav 3, Case 1"
...
# Starter med å gjøre dataframen til en tibble å kaller den new_df
new_df <- as_tibble(df$dataset)

#Gjør videre Tid variablen til en integer istedet for en string. 
new_df <- new_df %>%
  mutate(Tid = as.integer(Tid)) %>%
  rename(var = ContentsCode) %>% #Gir nye navn her
  rename(tid = Tid) %>%
  rename(verdi = value)
...
#Ved å sette faste priser så ser vi hvordan veksten har vært utenom inflasjonsforandringer osv. mens Løpende priser vil fortelle oss om hvordan det i gikk i norsk økonomi det året men med forskjell på valuta kurser som NOK. 
...
new_df %>%
  ...
  geom_line() +
  xlim(2000, 2021) +
  labs(title = "BNP per innbygger faste 2015 priser og løpende", # Legger til tittel
       caption = "Hentet fra SSB", # Legger til tekst under
       x = "År", y = "Brutto nasjonal produkt i tusener av kroner") #Navngir X og Y akse


#Figuren her viser hvordan faktisk vekst har vært i faste 2015 priser under "Memobnp" som er blå linje. Dette viser at vi ikke har hatt vi ikke har hatt så kraftig vekst som ved å bare se på BNP(Rød linje) vil tilsi, Ved å vise forskjell i vekst i løpende priser versus faste priser i år som for eks 2015 så får vi vist bedre hvordan økonomien har forandret seg år til år justert for inflasjon. 
...
#Denne koden skiller ut MEMOBNP fra BNP ved å legge til en ny rad slik at vi kan starte og mutere dataen. 
```
...
#Her gjør vi det mulig å beregne vekst siden vi nå får ny verdi og gammel verdi på samme tabell slik at vi kan regne med tallene i LBNP og LMEMOBNP siden.



# legger variablene i rekkefølge på tabell
...
#Her lager jeg ny df_endring og df_prosenter slik at jeg kan jobbe med begge for å komme bedre frem til svaret på siste oppgave. 
#jeg lar dette være med siden det hjalp meg veldig mye å jobbe å plages frem til løsningene her. 


df_prosenter <- df_endring %>%
  mutate(Pros_bnp = ((DBNP/BNP) * 100)) %>%
  mutate(Pros_memobnp = ((DMEMOBNP/MEMOBNP) * 100))



#Lager 2 nye variabler der de er i prosenter isteder.
...
df_long <- df_endring %>%
  pivot_longer(c( "BNP", "LBNP", "MEMOBNP", "LMEMOBNP", "DBNP", "DMEMOBNP"), names_to = "var", values_to = "bnp")
#Jeg gjør df_endring fra å være flere kolonner til å ha en lang kolonne slik at jeg kan plotte dette på en graf.
```
...
df_prosenter1 <- df_prosenter %>%
  pivot_longer(c("Pros_bnp", "Pros_memobnp"), names_to = "var", values_to = "bnp")
#Jeg lager en df_prosenter1 det jeg bare gir løpende priser og faste 2015 priser navnet "var" og gir de verdi bnp slik at jeg bare får de på grafen som plottes nedenfor. 



#Vi ser også tider med negativ vekst spesielt rundt finanskrisen og ved dotcom krasjet. Det kommer også senere rundt 2014 da det ble dårlig med oljepris som ledet til at mange i oljeindustrien mistet jobben og det var generelt mye nedbemanning. I 2020 så kom det korona som leder til et nytt kræsj men mye stimulus ledet til eksplosiv vekst. 
...
df_prosenter1 %>%
  ...
  geom_point() +
  geom_smooth()+
   labs(title = "Prosentvis endring", # Legger til tittel
       caption = "Hentet fra SSB", # Legger til tekst under
       x = "År", y = "Prosent") #Navngir X og Y akse
```

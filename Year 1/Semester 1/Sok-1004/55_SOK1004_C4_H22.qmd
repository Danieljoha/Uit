...
En god verdi er en som er leselig. Når det er binwidth=0.1 så er det svært vanskelig å se for mye miles per gallon de bilene får, men når man grupperer de ilag så får du en noe mer unøyaktig verdi men den er leselig og mer brukelig.
...
```{r}
oppgave2 <- df %>%
  rename(yrker = `næring (SN2007)`) %>% 
  rename(verdi = value) %>% 
  rename(kostnader = statistikkvariabel) %>% 
  rename(antall_ansatte = sysselsettingsgruppe) %>% 
...
list <- c("Industri", "Tjenesteyting", "Andre næringer")


oppgave3 <- oppgave2 %>%
  filter(yrker %in% list) %>% 
  filter(antall_ansatte>=20) %>% 
  filter(år >= 2015) %>% 
  filter(kostnader == "FoU-kostnader i alt") %>% 
  ggplot(aes(verdi)) +
  geom_histogram(binwidth = 0.4, fill = "purple", alpha=0.5) +
  theme_minimal() +
  facet_grid(yrker~antall_ansatte)+
  labs(x = "Verdi i milliarder")
oppgave3
```
...
oppgave4 <- oppgave2 %>%
  ...
  filter(antall_ansatte>=20) %>% 
  filter(år >= 2015) %>% 
  ggplot(aes(verdi, fill = kostnader )) +
  geom_histogram(binwidth = 0.4) +
  theme_minimal() +
  facet_grid(yrker~antall_ansatte)+
  labs(x = "Verdi i milliarder", subtitle = "Mengde sysselsatte")
oppgave4
```
